      ...
      (let ((loc (%opt-memory-location-key inst alias-roots)))
        (cond
          ((and loc (opt-memory-def-inst-p inst))
            (let ((vin version))
              (incf version)
              (setf (gethash inst annotations) (list :kind :def :location loc :in vin :out version))))
          ((and loc (opt-memory-use-inst-p inst))
            (setf (gethash inst annotations) (list :kind :use :location loc :in version :out version))))))))
  ...
  (let ((env (make-hash-table :test #'eq)))
    (dolist (inst (bb-instructions block))
      (when (typep inst '(or vm-jump vm-jump-zero vm-ret vm-halt))
        (return))
      (typecase inst
        (vm-const (%opt-memory-ssa-track-const inst env))
        (vm-move (%opt-memory-ssa-track-move inst env))
  ...
  (let ((reachable (make-hash-table :test #'eq))
    ...
    (loop while work
        ...
        (dolist (succ (bb-successors block))
          (when (and
              (%opt-memory-ssa-edge-feasible-p block succ)
...
(defun %opt-memory-ssa-merge-states (states)
  ...
  (cond
    ...
    ((null (cdr states)) (%opt-memory-ssa-copy-state (car states)))
    (t
      (let* ((first (car states))
        ...
        (maphash
          (lambda (key value)
            (unless (%opt-memory-ssa-key-agrees-p key value states)
              (push key dead)))
          first)
        (dolist (key dead)
          (remhash key merged))
...
(defun %opt-memory-ssa-state-equal (a b)
  (and
    ...
    (loop for key being the hash-keys of a
          always (multiple-value-bind (bv found-p) (gethash key b)
        (and found-p (eql (gethash key a) bv))))))

(defstruct (opt-memory-phi-node (:conc-name opt-memory-phi-)) "Explicit MemoryPhi node metadata for a block-entry location join.
...
FIRST-VERSION."
  (and
    (every
      (lambda (state)
        (multiple-value-bind (version found-p) (gethash loc state)
...
(defun %opt-memory-ssa-phi-version-for (block loc
    phi-version-table
    next-version)
...
allocated synthetic MemoryPhi version for BLOCK/LOC in PHI-VERSION-TABLE."
  (let* ((phi-key (list block loc))
    ...
    (if version (values version next-version)
      (let ((fresh (1+ next-version)))
        (setf (gethash phi-key phi-version-table) fresh)
        (values fresh fresh)))))

(defun %opt-memory-ssa-synthesize-entry-phis (block predecessor-states
    phi-version-table
    next-version)
        ...
        (when first-state
          (loop for loc being the hash-keys of first-state using (hash-value first-version)
                when (%opt-memory-ssa-loc-disagrees-p loc first-version rest-states)
                  do (multiple-value-bind (version new-next) (%opt-memory-ssa-phi-version-for block loc phi-version-table next-version)
...
(defun opt-memory-ssa-version-at (inst annotations &key (point :in))
  ...
  (when-let
    ((entry (gethash inst annotations)))
    (ecase point
      (:in (getf entry :in))
      (:out (getf entry :out)))))

(defun %opt-memory-ssa-track-move (inst env)
  (let ((dst (vm-dst inst))
        (src (vm-src inst)))
    (when dst
...
(defun %opt-memory-ssa-track-const (inst env)
  (let ((dst (vm-dst inst))
        (value (vm-value inst)))
    (when (and dst (integerp value))
      (setf (gethash dst env) value))))

(defun %opt-memory-ssa-key-agrees-p (key value states)
  (every
    (lambda (state)
