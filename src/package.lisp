...
(defpackage #:cl-cc/optimize
  ...
  (:shadowing-import-from #:cl-cc/vm
    ...
    #:*features* #:*load-pathname* #:*load-truename*
    #:*compile-file-pathname* #:*compile-file-truename*
    #:stream-external-format #:break #:inspect #:describe
    #:invoke-debugger #:step #:ed #:open #:close #:write-char #:read-char
    #:write-string #:read-line #:write-line #:terpri #:fresh-line
    #:finish-output #:force-output #:clear-output #:clear-input #:listen
    #:peek-char #:unread-char #:read-byte #:write-byte #:file-position
    #:file-length #:probe-file #:delete-file #:rename-file #:directory
    #:truename #:file-write-date #:DRIBBLE #:TRACE #:UNTRACE)
  ;; Every VM instruction type, constructor, and accessor this system
  ;; touches, plus the handful of CL-CC/VM-owned non-CL-shadowing symbols
  ;; (define-vm-instruction, instruction->sexp, sexp->instruction, the
  ;; Prolog pattern atoms neg/move/name/..., and vm-a/vm-b/vm-c generic
  ;; operand accessors). Generated by scanning src/*.lisp for every
  ;; identifier that resolved through the former blanket :use of
  ;; CL-CC/VM and is not already CL or handled above.
  (:import-from #:cl-cc/vm
    #:bswap #:dec #:define-vm-instruction #:div #:execute-instruction
    #:hash-table-values #:inc #:instruction->sexp #:make-vm-add #:make-vm-apply
    #:make-vm-aref #:make-vm-aset #:make-vm-ash #:make-vm-bswap #:make-vm-call #:make-vm-closure
    #:make-vm-concatenate #:make-vm-const #:make-vm-copy-vector #:make-vm-evenp #:make-vm-fill
    #:make-vm-fma #:make-vm-func-ref #:make-vm-get-global #:make-vm-hash-cons #:make-vm-integer-add
    #:make-vm-integer-mul #:make-vm-integer-mul-high-u #:make-vm-integer-sub #:make-vm-jump
    #:make-vm-jump-zero #:make-vm-label #:make-vm-le #:make-vm-logand #:make-vm-logcount
    #:make-vm-logior #:make-vm-logxor #:make-vm-lt #:make-vm-make-closure #:make-vm-min
    #:make-vm-move #:make-vm-mul #:make-vm-neg #:make-vm-oddp #:make-vm-prefetch #:make-vm-ret
    #:make-vm-rotate #:make-vm-select #:make-vm-simd-vector-op #:make-vm-slot-read
    #:make-vm-slot-write #:make-vm-string-length #:make-vm-sub #:make-vm-tail-call #:move
    #:neg #:rotate-right #:sexp->instruction #:vm-a #:vm-abs
    #:vm-acos-inst #:vm-add #:vm-add-checked #:vm-add-package-local-nickname #:vm-adjustable-reg
    #:vm-and #:vm-apply #:vm-aref #:vm-aref-multi #:vm-args #:vm-array-adjustable-p
    ...
    #:vm-closure-optional-params #:vm-closure-p #:vm-closure-params #:vm-closure-ref-idx
    #:vm-closure-reg #:vm-closure-rest-param #:vm-closure-rest-stack-alloc-p #:vm-clrhash
    #:vm-coerce-to-list #:vm-coerce-to-string #:vm-coerce-to-vector #:vm-concatenate #:vm-cons
    #:vm-cons-p #:vm-cons-reg #:vm-const #:vm-const-p #:vm-const-value #:vm-copy-vector
    #:vm-cos-inst #:vm-dec #:vm-decode-universal-time #:vm-denominator #:vm-displaced-to-reg
    #:vm-div #:vm-dst #:vm-dst-array-reg #:vm-element-type-reg #:vm-encode-universal-time #:vm-endp
    #:vm-env-regs #:vm-eq #:vm-error-instruction #:vm-establish-catch #:vm-establish-handler
    #:vm-eval #:vm-exp-inst #:vm-fboundp #:vm-fifth #:vm-fill #:vm-fill-pointer-reg #:vm-first
    #:vm-float-add #:vm-float-div #:vm-float-mul #:vm-float-precision #:vm-float-sign
    #:vm-float-sub #:vm-floor-inst #:vm-fma #:vm-format-inst #:vm-fourth #:vm-func-name
    #:vm-func-ref #:vm-func-reg #:vm-function-p #:vm-gcd #:vm-ge #:vm-generic-add #:vm-generic-call
    #:vm-generic-div #:vm-generic-eq #:vm-generic-gt #:vm-generic-lt #:vm-generic-mul
    #:vm-generic-sub #:vm-get-global #:vm-get-internal-real-time #:vm-get-internal-run-time
    #:vm-get-universal-time #:vm-gethash #:vm-gethash-eq #:vm-gethash-eql #:vm-gethash-equal
    #:vm-gf-reg #:vm-global-name #:vm-global-vars #:vm-gt #:vm-halt #:vm-handler-label
    #:vm-hash-cons #:vm-hash-key #:vm-hash-table-count #:vm-hash-table-keys #:vm-hash-table-reg
    #:vm-hash-table-rehash-size #:vm-hash-table-rehash-threshold #:vm-hash-table-size
    #:vm-hash-table-test #:vm-hash-table-values #:vm-hash-value #:vm-inc #:vm-index #:vm-index-reg
    #:vm-initarg-regs #:vm-initial-element #:vm-instruction #:vm-integer-add #:vm-integer-length
    #:vm-integer-mul #:vm-integer-mul-high-s #:vm-integer-mul-high-u #:vm-integer-p
    #:vm-integer-sub #:vm-jump #:vm-jump-p #:vm-jump-zero #:vm-label #:vm-label-name #:vm-label-p
    #:vm-last #:vm-lcm #:vm-le #:vm-len-reg #:vm-length #:vm-lhs #:vm-list-length #:vm-listp
    #:vm-log-inst #:vm-logand #:vm-logbitp #:vm-logcount #:vm-logeqv #:vm-logior #:vm-lognot
    #:vm-logtest #:vm-logxor #:vm-lt #:vm-make-array #:vm-make-closure #:vm-make-closure-p
    #:vm-make-list #:vm-make-obj #:vm-make-random-state #:vm-make-string #:vm-makunbound #:vm-max
    #:vm-member #:vm-method-qualifier #:vm-method-reg #:vm-method-specializer #:vm-min #:vm-mod
    #:vm-move #:vm-move-dst #:vm-move-src #:vm-mul #:vm-mul-checked #:vm-mv-bind #:vm-mv-bind-regs
    #:vm-name #:vm-neg #:vm-next-method-p #:vm-not #:vm-nth #:vm-nthcdr #:vm-null #:vm-null-p
    #:vm-num-eq #:vm-number-p #:vm-numerator #:vm-obj-reg #:vm-or #:vm-osr-entry #:vm-parts
    #:vm-pop #:vm-prefetch #:vm-prefetch-base-reg #:vm-prefetch-index-reg #:vm-prefetch-kind
    #:vm-prefetch-locality #:vm-prefetch-offset #:vm-prefetch-scale #:vm-print #:vm-push
    #:vm-random #:vm-rational #:vm-rationalize #:vm-read-char #:vm-read-line #:vm-reg #:vm-reg-get
    ...
    #:vm-spread-values #:vm-sqrt #:vm-src #:vm-src-array-reg #:vm-src-regs #:vm-str1 #:vm-str2
    #:vm-string-coerce #:vm-string-downcase #:vm-string-length #:vm-string-reg #:vm-string-upcase
    #:vm-stringp #:vm-sub #:vm-sub-checked #:vm-subseq #:vm-superclasses #:vm-svref #:vm-symbol-p
    #:vm-sync-handler-regs #:vm-tail-call #:vm-tan-inst #:vm-third #:vm-throw #:vm-trampoline
    #:vm-truncate #:vm-type-name #:vm-type-of #:vm-typep #:vm-val-reg #:vm-value #:vm-value-reg
    #:vm-values #:vm-values-regs #:vm-values-to-list #:vm-values-typep #:vm-vectorp #:vm-warn
    #:vm-write-char #:vm-write-string)
  (:export
   ...
   #:cfg-compute-loop-depths
    ...
    #:make-opt-shape-transition-cache
     ...
     #:opt-tier-transition #:opt-tier-runtime-state #:make-opt-tier-runtime-state
     #:opt-tier-runtime-state-tier #:opt-tier-runtime-state-call-count
     #:opt-tier-runtime-state-backedge-count #:opt-tier-runtime-state-pgo-handoff-p
     #:opt-tier-record-runtime-event #:opt-make-pure-function-runtime-memoizer
