...
(defun %opt-lsc-same-base-p (a b alias-roots)
  ...
  (or
    (eql a b)
    (and alias-roots (hash-table-p alias-roots) (opt-must-alias-p a b alias-roots))))
      ...
      (let ((offset (cl-cc/vm:vm-slot-write-slot-name inst)))
        (when (integerp offset)
          (values
            ...
            offset
            (cl-cc/vm:vm-slot-write-value-reg inst)
            inst))))))
...
(defun %opt-lsc-compatible-access-p (first next kind family expected-offset alias-roots)
  ...
  (multiple-value-bind (next-kind next-family next-base next-offset) (%opt-lsc-access next)
    (multiple-value-bind (_ first-family first-base) (%opt-lsc-access first)
      (declare (ignore _))
      (and
        (eq next-kind kind)
...
If the first two accesses cannot be widened/coalesced, NIL is returned."
  (multiple-value-bind (kind family _base start-offset) (%opt-lsc-access (first instructions))
    ...
    (when (and kind family)
      (let ((raw (list (first instructions)))
        ...
        (loop for inst in (rest instructions)
              while (and
            (< (length raw) +opt-coalesce-max-lanes+)
            (%opt-lsc-compatible-access-p (first raw) inst kind family expected alias-roots))
              do (push inst raw) (incf expected))
        (let* ((ordered (nreverse raw))
               (best-len
              (loop for n from (length ordered) downto 2
                    when (and (%opt-lsc-power-of-two-p n) (%opt-lsc-natural-alignment-p start-offset n))
                      return n)))
          (when best-len
...
(defun %opt-lsc-extract-loads (group fresh)
  ...
  (multiple-value-bind (_ family base start-offset _value first-inst) (%opt-lsc-access (first group))
    (declare (ignore _ _value))
    (let* ((wide-reg (funcall fresh))
           (mask-reg (funcall fresh))
           (out
          (list
            (%opt-lsc-make-load family wide-reg base start-offset first-inst)
            (make-vm-const :dst mask-reg :value #xff))))
      (loop for inst in group
            for lane from 0
            do (setf out (%opt-lsc-extract-load-lane inst lane wide-reg mask-reg fresh out)))
      (nreverse out))))
      ...
      (let ((lane-reg masked-reg))
        (when (plusp lane)
          (let ((shift-reg (funcall fresh))
            ...
            (push
              (make-vm-const :dst shift-reg :value (* lane +opt-coalesce-lane-bits+))
              out)
            (push (make-vm-ash :dst shifted-reg :lhs masked-reg :rhs shift-reg) out)
            (setf lane-reg shifted-reg)))
        (if packed-reg (let ((next-packed (funcall fresh)))
...
(defun %opt-lsc-pack-stores (group fresh)
  ...
  (multiple-value-bind (_ family base start-offset _value first-inst) (%opt-lsc-access (first group))
    ...
    (let ((mask-reg (funcall fresh))
      ...
      (loop for inst in group
            for lane from 0
            do (multiple-value-bind (new-out new-packed) (%opt-lsc-pack-store-lane inst lane mask-reg packed-reg fresh out)
          (setf out new-out
                packed-reg new-packed)))
...
marker."
  (multiple-value-bind (kind-a family-a base-a offset-a) (%opt-lsc-access a)
    (multiple-value-bind (kind-b family-b base-b offset-b) (%opt-lsc-access b)
      (and
        (eq kind-a :load)
        (eq kind-b :load)
...
groups until the VM grows an explicit unaligned access marker."
  (let ((fresh (%opt-fresh-register-generator instructions))
    ...
    (labels ((walk (rest acc changed)
            ...
            (t
              (if-let
                ((group (%opt-lsc-group-prefix rest aliases)))
                (walk
                  (nthcdr (length group) rest)
...
(defun %opt-lsc-extract-load-lane (inst lane wide-reg mask-reg fresh out)
  (multiple-value-bind (_kind _family _base _offset dst) (%opt-lsc-access inst)
    (declare (ignore _kind _family _base _offset))
