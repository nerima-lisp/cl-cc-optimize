    ...
    (dolist (inst instructions)
      (typecase inst
        ...
        ((or vm-jump vm-jump-zero vm-ret vm-halt)
          (%mps-flush-all state)
          (%mps-emit state inst))
        (vm-get-global (%opt-mps-forward-get-global inst state))
        (vm-slot-read (%opt-mps-forward-slot-read inst state alias-roots))
        (vm-set-global
          (%mps-remember-store
            state
            (list :global (cl-cc/vm:vm-set-global-name inst))
            inst))
        (vm-slot-write
          (%mps-remember-store
            state
            (opt-slot-alias-key
              (cl-cc/vm:vm-slot-write-obj-reg inst)
              (cl-cc/vm:vm-slot-write-slot-name inst)
              alias-roots)
            inst))
        (t
          (when-let ((dst (opt-inst-dst inst))) (%mps-flush-dependent-on-reg state dst))
...
(defun %available-store-state-equal (a b)
  ...
  (and
    ...
    (let ((same t))
      (maphash
        (lambda (key value)
          (multiple-value-bind (other found-p) (gethash key b)
...
(defun %available-store-merge-states (states)
  ...
  (cond
    ((null states) (make-hash-table :test #'equal))
    ((null (cdr states)) (%available-store-copy-state (car states)))
    (t
      (let* ((first (car states))
        ...
        (maphash
          (lambda (key value)
            (unless (%available-store-key-agrees-p key value states)
              (push key dead)))
          first)
        (dolist (key dead)
          (remhash key merged))
  ...
  (typecase inst
    ...
    ((or vm-slot-write vm-slot-read)
      (typecase inst
        ...
        (vm-slot-read
          (opt-slot-alias-key
            (cl-cc/vm:vm-slot-read-obj-reg inst)
...
(defun %available-store-kill-dependent-on-reg (state reg &key exclude-key)
  ...
  (when reg
    (let ((dead nil))
      (maphash
        (lambda (key store)
          (when (and store (not (equal key exclude-key)) (%mps-pending-uses-reg-p store reg))
            (push key dead)))
        state)
      (dolist (key dead)
        (remhash key state)))))
...
(define-inst-type-predicate
  %available-store-control-inst-p
  (or vm-jump vm-jump-zero vm-ret vm-halt)
...
(defun %available-store-clobber-inst-p (inst)
  ...
  (not
    (or
      (typep inst '(or vm-set-global))
      (%available-store-control-inst-p inst)
      (opt-inst-pure-p inst))))

(defun %available-store-transfer (block in-state
    alias-roots)
      ...
      (typecase inst
        (vm-get-global
          (let* ((key (%available-store-location-key inst alias-roots))
            ...
            (if store (%available-store-kill-dependent-on-reg state dst :exclude-key key)
  ...
  (typecase inst
    (vm-get-global
      (when (typep store 'vm-set-global)
        (make-vm-move
          :dst
          (cl-cc/vm:vm-get-global-dst inst)
          :src
          (cl-cc/vm:vm-set-global-src store))))
    (vm-slot-read
      (when (typep store 'vm-slot-write)
        (make-vm-move
          :dst
          (cl-cc/vm:vm-slot-read-dst inst)
          :src
          (cl-cc/vm:vm-slot-write-value-reg store))))))
...
Conservative straight-line scan: tracks vm-const and vm-move aliases within BLOCK."
  (let ((env (make-hash-table :test #'eq)))
    (dolist (inst (bb-instructions block))
      (typecase inst
        (vm-const (%available-store-track-const inst env))
        (vm-move (%available-store-track-move inst env))
        (t (when-let ((dst (opt-inst-dst inst))) (remhash dst env)))))
    (gethash reg env)))
    ...
    (cond
      ((typep term 'vm-jump-zero)
        (let* ((cond-reg (vm-reg term))
          ...
          (if const-value (if (zerop const-value) zero-edge-p
              (not zero-edge-p))
            t)))
      (t t))))

(defun %available-store-refined-entry-state (block flow
    reachable)
  ...
  (let* ((preds (bb-predecessors block))
        ...
        (remove-if-not
          (lambda (pred)
            (and (gethash pred reachable) (%available-store-edge-feasible-p pred block)))
          preds))
         (states
        (mapcar
          (lambda (pred)
  ...
  (let ((reachable (make-hash-table :test #'eq))
    ...
    (loop while work
        ...
        (dolist (succ (bb-successors block))
          (when (and
              (%available-store-edge-feasible-p block succ)
              (not (gethash succ reachable)))
            (setf (gethash succ reachable) t)
            (push succ work)))))
    reachable))

(defun %available-store-build-rewrite-map (block entry-state
    alias-roots)
...
(defun %opt-pass-store-to-load-forward-cfg (instructions)
  ...
  (let* ((cfg (cfg-build instructions))
        ...
        (opt-run-dataflow
          ...
          :forward
          :meet
          #'%available-store-merge-states
          :transfer
          (lambda (block in-state)
            (%available-store-transfer block in-state alias-roots))
          :state-equal
          #'%available-store-state-equal
          :initial-state
          initial
          :boundary-state
          initial
          :copy-state
          #'%available-store-copy-state)))
      ...
      (loop for block across (cfg-blocks cfg)
            when (and block (gethash block reachable))
              do (let* ((entry-state (%available-store-refined-entry-state block flow reachable))
               (per-block (%available-store-build-rewrite-map block entry-state alias-roots)))
          (maphash
            (lambda (old new)
...
incoming path to the load's basic block."
  (let ((cfg (cfg-build instructions)))
    (if (= (length (cfg-blocks cfg)) 1) (%opt-pass-store-to-load-forward-linear instructions)
      (%opt-pass-store-to-load-forward-cfg instructions))))

(defun %available-store-track-move (inst env)
  (let ((dst (vm-dst inst))
        (src (vm-src inst)))
    (when dst
      (if src (multiple-value-bind (value found-p) (gethash src env)
          (if found-p (setf (gethash dst env) value)
            (remhash dst env)))
        (remhash dst env)))))

(defun %available-store-track-const (inst env)
  (let ((dst (vm-dst inst))
        (value (vm-value inst)))
    (when (and dst (integerp value))
      (setf (gethash dst env) value))))

(defun %available-store-key-agrees-p (key value states)
  (every
    (lambda (state)
      (multiple-value-bind (other found-p) (gethash key state)
        (and found-p (eq value other))))
    (cdr states)))

(defun %opt-mps-forward-get-global (inst state)
...
(defun %opt-mps-forward-slot-read (inst state alias-roots)
  (let* ((key
        (opt-slot-alias-key
          (cl-cc/vm:vm-slot-read-obj-reg inst)
          (cl-cc/vm:vm-slot-read-slot-name inst)
