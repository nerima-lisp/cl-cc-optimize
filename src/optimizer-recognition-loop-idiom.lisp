...
(defun %opt-fill-window-shape-valid-p (len-inst init header cmp exit-jump store one inc step back-jump exit-label)
  "T when the fixed-role instructions in a candidate fill-loop window have the
   expected vm instruction types and immediate values for the canonical
   zero-based array fill idiom."
  (and (typep len-inst 'vm-array-length)
       (typep init 'vm-const)
       (eql (vm-value init) 0)
...
(defun opt-fill-recognition-match-at (instructions pos)
  ...
  (let ((end (+ pos 10)))
    (when (and (plusp pos)
      ...
      (destructuring-bind (len-inst init header cmp exit-jump store one inc step back-jump exit-label)
          (subseq instructions (1- pos) end)
        (when (%opt-fill-window-shape-valid-p len-inst init header cmp exit-jump store one inc step back-jump exit-label)
...
(defun %opt-copy-window-shape-valid-p (init header cmp exit-jump load store one inc step back-jump exit-label)
  "T when the fixed-role instructions in a candidate copy-loop window have the
   expected vm instruction types and immediate values for the canonical
   zero-based array copy idiom."
  (and (typep init 'vm-const)
       (eql (vm-value init) 0)
       (typep header 'vm-label)
       (typep cmp 'vm-lt)
       (typep exit-jump 'vm-jump-zero)
  ...
  (let ((end (+ pos 11)))
    (when (and (plusp pos)
      ...
      (destructuring-bind (len-inst init header cmp exit-jump load store one inc step back-jump exit-label)
          (subseq instructions (1- pos) end)
        (when (%opt-copy-window-shape-valid-p init header cmp exit-jump load store one inc step back-jump exit-label)
