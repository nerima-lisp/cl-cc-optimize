...
(defun %opt-verified-reciprocal-div-exact-p (divisor multiplier shift lo hi)
  ...
  (loop for x from lo to hi
        always (= (floor x divisor) (ash (* x multiplier) (- shift)))))
  ...
  (when (and
    ...
    (let ((lo (opt-interval-lo interval))
      ...
      (when (and
        ...
        (%opt-verified-reciprocal-search divisor lo hi)))))
...
supports intervals that include negative dividends."
  (when (and interval (integerp divisor) (> divisor 1) (not (opt-power-of-2-p divisor)))
    (let ((lo (opt-interval-lo interval))
          (hi (opt-interval-hi interval)))
      (when (and
          (integerp lo)
...
are limited to vm-const, vm-mul, and vm-ash as required by FR-282."
  (when-let
    ((params (%opt-find-verified-reciprocal-div-params divisor interval)))
    (destructuring-bind (multiplier shift) params
      (let ((mult-reg (funcall new-reg-fn))
        ...
        (list
          (make-vm-const :dst mult-reg :value multiplier)
...
instructions are limited to vm-const, vm-mul, vm-add, and vm-ash."
  (when-let
    ((params (%opt-find-verified-reciprocal-div-params-with-bias divisor interval)))
...
Otherwise it is:
  ...
  (when (and
    ...
    (let* ((word-bits +opt-strength-reduce-word64-bits+)
      ...
      (loop (incf p) (setf q1 (* 2 q1)
              r1 (* 2 r1)) (when (>= r1 nc)
          (incf q1)
          (decf r1 nc)) (setf q2 (* 2 q2)
              r2 (1+ (* 2 r2))) (when (>= r2 divisor)
          (incf q2)
          (decf r2 divisor)) (let ((delta (- divisor 1 r2)))
          (unless (and (< p (* 2 word-bits)) (or (< q1 delta) (and (= q1 delta) (zerop r1))))
            (let* ((magic (1+ q2))
                   (add-adjust-p (>= magic two-word))
                   (multiplier
                  (if add-adjust-p (- magic two-word)
                    magic))
                   (shift (- p word-bits)))
...
(defun %opt-emit-final-right-shift (dst src shift new-reg-fn)
  ...
  (if (zerop shift) (list (make-vm-move :dst dst :src src))
...
that fixed-width lowering. Unknown, signed, or wider values remain on vm-div."
  (when (%opt-word64-nonnegative-interval-p interval)
    (when-let
      ((params (%opt-find-unsigned-magic-div-params divisor)))
      (destructuring-bind (multiplier shift add-adjust-p) params
        (let ((mult-reg (funcall new-reg-fn))
          ...
          (append
            ...
            (if add-adjust-p (let ((diff-reg (funcall new-reg-fn))
                    (one-reg (funcall new-reg-fn))
                    (half-reg (funcall new-reg-fn))
                    (sum-reg (funcall new-reg-fn)))
                (append
                  (list
                    (make-vm-sub :dst diff-reg :lhs src :rhs high-reg)
...
Returns a list of vm instructions (may include vm-const, vm-ash, vm-add, vm-move, vm-neg)."
  (let* ((negp (minusp n))
         (absn (abs n)))
    (cond
      ((zerop absn) (list (make-vm-const :dst dst :value 0)))
      ((= absn 1)
        (cons
          (make-vm-move :dst dst :src src)
          (when negp
            (list (make-vm-neg :dst dst :src dst)))))
      (t (%opt-mul-by-const-shift-add-seq dst src absn negp new-reg-fn)))))
    ...
    (cond
      ((and rv (opt-power-of-2-p rv))
        (let* ((k (- (1- (integer-length rv))))
          ...
          (funcall emit (make-vm-const :dst shift-reg :value k))
          (funcall emit (make-vm-ash :dst dst :lhs lhs :rhs shift-reg))))
      ((and rv (integerp rv) (> rv 1) (not (opt-power-of-2-p rv)))
        (if-let
          ((seq
              (or
                (%opt-div-by-verified-reciprocal-seq dst lhs rv lhs-interval new-reg)
          ...
          (progn
            (when-let ((dstreg (opt-inst-dst inst))) (remhash dstreg env))
            (funcall emit inst))))
      (t
        (when-let ((dstreg (opt-inst-dst inst))) (remhash dstreg env))
        (funcall emit inst)))))
      ...
      (dolist (inst instructions)
        (typecase inst
          ...
          (vm-mod
            ...
            (%opt-transfer-interval-inst inst intervals))
          (t
            (when-let ((dst (opt-inst-dst inst))) (remhash dst env))
            (emit inst)
            (%opt-transfer-interval-inst inst intervals)))))
    (nreverse result)))

;;; Bswap and rotate recognition passes are in optimizer-recognition.lisp.
(defun %opt-verified-reciprocal-search (divisor lo hi)
  (let ((max-shift (max 1 (1+ (integer-length (* (max 1 hi) (1- divisor)))))))
    (loop for shift from 1 to max-shift
...
(defun %opt-mul-by-const-shift-add-seq (dst src absn negp new-reg-fn)
  (let ((bits
        (loop for bit from 0 below (integer-length absn)
              when (logbitp bit absn)
                collect bit))
        (seq nil)
        (terms nil))
    (dolist (bit bits)
      (if (zerop bit) (push src terms)
