...
(defun opt-pass-specialize-known-args (instructions)
  ...
  (let* ((func-defs (opt-collect-function-defs instructions))
    ...
    (labels ((clear-dst-tracks (dst)
             ...
             (emit-specialized-clone (callee-label params body const-bindings
                                      specialized-label dynamic-params clone-reg)
               ;; First call site for SPECIALIZED-LABEL: run the actual partial
               ;; evaluation and splice the residual clone's closure/label/body
               ;; into the output stream once, ahead of its first call site.
               (let* ((partial
             ...
             (handle-known-callee-call (inst)
               ;; Rewrite INST to call a constant-argument specialization of its
               ;; callee when the callee is statically known and a specialization
