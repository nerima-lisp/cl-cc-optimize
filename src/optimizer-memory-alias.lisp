...
(defun %opt-tbaa-kind (reg type-facts)
  ...
  (cond
    ((hash-table-p type-facts) (gethash reg type-facts))
    ((and (consp type-facts) (keywordp (first type-facts))) (getf type-facts reg))
    ((consp type-facts) (cdr (assoc reg type-facts :test #'eq)))))
...
(defun opt-memory-unknown-write-inst-p (inst)
  ...
  (or
    (typep inst '(or vm-call vm-tail-call vm-generic-call vm-apply))
...
treat absent instruction families as non-matches."
  (or
    (and (find-class type nil) type)
    (multiple-value-bind (vm-type status) (find-symbol (symbol-name type) :cl-cc/vm)
...
(defun opt-build-memory-tbaa-metadata (instructions)
  "Build reusable TBAA metadata for memory instructions in INSTRUCTIONS."
  (let ((metadata (make-hash-table :test (function eq)))
        (alias-roots (opt-compute-heap-aliases instructions))
        (type-facts (opt-compute-heap-type-facts instructions)))
    (setf (gethash :alias-roots metadata) alias-roots
          (gethash :type-facts metadata) type-facts)
    (dolist (inst instructions metadata)
      (when (or (opt-memory-read-inst-p inst) (opt-memory-write-inst-p inst))
        (setf (gethash inst metadata) (list
            :read-p
            (opt-memory-read-inst-p inst)
            :write-p
            (opt-memory-write-inst-p inst)))))))

(defun opt-memory-tbaa-metadata (inst metadata)
  "Return the TBAA entry for INST from METADATA, or NIL."
  (gethash inst metadata))
