...
(defstruct (polyhedral-domain (:conc-name poly-domain-)) dimensions
  constraints)

(defstruct (polyhedral-access (:conc-name poly-access-)) array-reg
  write-p
  coefficients
  offset)

(defstruct (polyhedral-statement (:conc-name poly-stmt-)) loops
  domain
  accesses
  schedule
  body)
...
(defun %poly-swap-first-two (list)
  (if (and (consp list) (consp (cdr list))) (list* (second list) (first list) (cddr list))
    list))

(defun %poly-swap-coefficients (coefficients)
  (if (and (consp coefficients) (consp (cdr coefficients))) (list* (second coefficients) (first coefficients) (cddr coefficients))
    coefficients))
  ...
  (if (%poly-interchange-beneficial-p statement) (let* ((domain (poly-stmt-domain statement))
          ...
          (and
            ...
            (make-polyhedral-domain
              :dimensions
              (%poly-swap-first-two (poly-domain-dimensions domain))
              :constraints
              (poly-domain-constraints domain))))
           (new-accesses
          (mapcar
            (lambda (access)
              (make-polyhedral-access
                :array-reg
                (poly-access-array-reg access)
                :write-p
                (poly-access-write-p access)
                :coefficients
                (%poly-swap-coefficients (poly-access-coefficients access))
                :offset
                (poly-access-offset access)))
            (poly-stmt-accesses statement))))
      (make-polyhedral-statement
        :loops
        (%poly-swap-first-two (poly-stmt-loops statement))
        :domain
        new-domain
        :accesses
        new-accesses
        :schedule
        (list :interchanged t :previous (poly-stmt-schedule statement))
        :body
        (poly-stmt-body statement)))
    statement))
...
(define-inst-type-predicate
  %poly-unsafe-side-effect-p
  (or
    vm-call
    vm-tail-call
    vm-trampoline
...
(defun %poly-affine-read-accesses (instructions inner outer)
  (multiple-value-bind (const-env def-env) (%loop-fr514-build-envs instructions (opt-loop-head-index outer))
      ...
      (let ((inner-accesses nil)
        ...
        (dolist (inst core)
          (when (typep inst 'vm-aref)
            (push
              (%loop-fr514-affine-access inst (opt-loop-iv-reg inner) const-env def-env)
              inner-accesses)
            (push
              (%loop-fr514-affine-access inst (opt-loop-iv-reg outer) const-env def-env)
              outer-accesses)))
        (values
          (remove nil inner-accesses)
          (remove nil outer-accesses)
          (count-if
            (lambda (inst)
              (typep inst 'vm-aref))
            core))))))
  ...
  (let* ((vec (coerce instructions 'vector))
    ...
    (loop while (< i n)
        ...
        (if outer (let ((inner (%poly-nested-loop-at vec outer)))
            (if (and inner (%poly-loop-interchange-candidate-p instructions vec outer inner)) (setf out (%poly-emit-interchanged-loop vec outer inner out)
                    changed t
...
OPT-PASS-POLYHEDRAL.  Descriptor inputs receive an explicit tiled schedule."
  (let* ((sizes
        (or
          tile-sizes
          (list *polyhedral-default-tile-size* *polyhedral-default-tile-size*)))
         (sizes
        (if (listp sizes) sizes
          (list sizes sizes))))
    (if (and
        (polyhedral-statement-p statement)
        (>= (length (poly-stmt-loops statement)) 2)) (make-polyhedral-statement
        :loops
        (poly-stmt-loops statement)
        :domain
        (poly-stmt-domain statement)
        :accesses
        (poly-stmt-accesses statement)
        :schedule
        (list
          ...
          t
          :tile-sizes
          (subseq (append sizes sizes) 0 2)
          :previous
          (poly-stmt-schedule statement))
        :body
        (poly-stmt-body statement))
      statement)))
...
shared schedule.  Non-descriptor or incompatible inputs are returned unchanged."
  (if (%polyhedral-fusable-p statements) (let ((first (first statements)))
      (make-polyhedral-statement
        :loops
        (poly-stmt-loops first)
        :domain
        (poly-stmt-domain first)
        :accesses
        (mapcan
          (lambda (stmt)
            (copy-list (poly-stmt-accesses stmt)))
          statements)
        :schedule
        (list :fused t :count (length statements) :previous (poly-stmt-schedule first))
        :body
        (mapcan
          (lambda (stmt)
            (copy-list (poly-stmt-body stmt)))
          statements)))
    statements))
...
(defun %poly-emit-tile-metadata (vec outer inner tile-size out)
  (push
    (make-vm-label
      :name
      (%poly-interchanged-label
        (opt-loop-head-label outer)
...
(defun %poly-tile-instructions (instructions &key (tile-size *polyhedral-default-tile-size*))
  ...
  (let* ((vec (coerce instructions 'vector))
    ...
    (loop while (< i n)
          do (let ((outer (%loop-fr514-parse-canonical-loop-at vec i)))
        (if outer (let ((inner (%poly-nested-loop-at vec outer)))
            (if (and inner (%poly-loop-interchange-candidate-p instructions vec outer inner)) (setf out (%poly-emit-tile-metadata vec outer inner tile-size out)
                    changed t
                    i (1+ (opt-loop-exit-index outer)))
              (progn
                (loop for k from (opt-loop-head-index outer) to (opt-loop-exit-index outer)
...
eligible nests."
  (if *polyhedral-enabled* (%poly-tile-instructions (polyhedral-loop-interchange instructions))
    instructions))

(defun %polyhedral-fusable-p (statements)
  (and
    (consp statements)
    (every #'polyhedral-statement-p statements)
    (let ((loops (%polyhedral-loops-prefix (first statements)))
          (domain (poly-stmt-domain (first statements))))
      (every
        (lambda (stmt)
          (%polyhedral-stmt-compatible-p loops domain stmt))
        statements))))

(defun %polyhedral-stmt-compatible-p (loops domain stmt)
  (and
    (equal loops (%polyhedral-loops-prefix stmt))
    (equal
      (and domain (poly-domain-dimensions domain))
      (and (poly-stmt-domain stmt) (poly-domain-dimensions (poly-stmt-domain stmt))))))

(defun %polyhedral-loops-prefix (stmt)
  (subseq (poly-stmt-loops stmt) 0 (min 2 (length (poly-stmt-loops stmt)))))
