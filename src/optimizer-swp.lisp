...
(define-inst-type-predicate
  %opt-swp-loop-cmp-p
  (or vm-lt vm-le vm-gt vm-ge vm-eq vm-num-eq)
...
(defun %opt-swp-generated-p (instructions)
  ...
  (%opt-generated-label-present-p
    instructions
    *opt-software-pipeline-label-prefix*))
    ...
    (let* ((header (aref vec i))
      ...
      (when (and exit-index (> exit-index (+ i 4)))
        (let* ((back-index (1- exit-index))
               ...
               (body
              (loop for k from (+ i 3) below back-index
                    collect (aref vec k)))
               (step (car (last body))))
          (when (and
              (typep back 'vm-jump)
...
it at the end of the kernel."
  (let ((core (butlast body)))
    (if (%opt-swp-safe-core-p core) (%opt-swp-schedule-core core issue-width)
...
idempotent in the convergence pipeline."
  (if (%opt-swp-generated-p instructions) instructions
    (let* ((vec (coerce instructions 'vector))
      ...
      (loop while (< i n)
          ...
          (if lp (multiple-value-bind (seq rewritten-p) (%opt-swp-rewrite-loop vec lp counter)
              (dolist (inst seq)
                (push inst out))
              (when rewritten-p
                (incf counter)
...
(defun %opt-swp-schedule-core (core issue-width)
  (let* ((ddg (cfg-build-ddg core :latency-fn #'%opt-inst-latency))
         (mii (cfg-compute-mii ddg :issue-width issue-width))
         (scheduled
        (sort
          (loop for node across ddg
                collect (list
              :inst
              (opt-ddg-node-inst node)
              :index
              (opt-ddg-node-index node)
              :slot
              (mod (opt-ddg-node-index node) mii)
              :latency
              (opt-ddg-node-latency node)))
          (lambda (a b)
            (%opt-swp-schedule-entry< a b)))))
    (values
      (mapcar
        (lambda (entry)
          (getf entry :inst))
...
(defun %opt-swp-schedule-entry< (a b)
  (or
    (< (getf a :slot) (getf b :slot))
    (and (= (getf a :slot) (getf b :slot)) (> (getf a :latency) (getf b :latency)))
    (and
      (= (getf a :slot) (getf b :slot))
